...
Report bugs at https://github.com/qucontrol/newtonprop/issues.
...
The best way to send feedback is to file an issue at https://github.com/qucontrol/newtonprop/issues.
...
3. Check https://travis-ci.org/qucontrol/newtonprop/pull_requests
...
Ready to contribute? Follow `Aaron Meurer's Git Workflow Notes`_ (with ``qucontrol/newtonprop`` instead of ``sympy/sympy``)

In short,

1. Clone the repository from ``git@github.com:qucontrol/newtonprop.git``
...
Python versions.
You still can (and should) look at https://travis-ci.org/qucontrol/newtonprop/ to check that your commits pass all tests.
...
newtonprop includes a full test-suite using pytest_.
We strive for a `test coverage`_ above 90%.


From a checkout of the ``newtonprop`` repository, assuming conda_ is installed, you can use
...
.. _test coverage: https://coveralls.io/github/qucontrol/newtonprop?branch=master
...
newtonprop could always use more documentation, whether
...
At any point, from a checkout of the ``newtonprop`` repository (and
...
``newtonprop``, and that you have successfully run ``make test`` (which creates
...
To run e.g. only the tests defined in ``tests/test_newtonprop.py``, use::

    $ ./.venv/py37/bin/pytest tests/test_newtonprop.py
